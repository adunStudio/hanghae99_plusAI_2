...
class ImageChatService:
    def __init__(self, st, api_key):
        ...
        if 'waiting' not in self.st.session_state:
            self.st.session_state.waiting = False

        if "messages" not in self.st.session_state:
            self.st.session_state.messages = []
            self.st.session_state.messages.append(SystemMessage(content='당신은 주어지는 이미지를 참고해서 응답하는 챗봇입니다.'))
            self.st.session_state.messages.append(AIMessage(content='안녕하세요! 무엇을 도와드릴까요?'))
    ...
    def set_waiting_true(self):
        self.st.session_state.waiting = True
...
if uploaded_images := st.file_uploader("함께할 이미지를 올려주세요!", type=['png', 'jpg', 'jpeg'], accept_multiple_files=True):
    ...
    if have_human_message == False:
        ...
        if right.button("주어진 두 사진의 차이점이 뭐야?", icon=":material/mood:", use_container_width=True, disabled=chat_service.waiting, on_click=chat_service.set_waiting_true):
            selected_prompt = '주어진 두 사진의 차이점이 뭐야?'


    prompt = st.chat_input("메시지를 입력하세요.", disabled=chat_service.waiting, on_submit=chat_service.set_waiting_true)

    if selected_prompt is not None or prompt is not None:
